...
export default function FloorDetail() {
  ...
  const { floors, addRoom, addStudent, deleteStudent, updateStudent, getBlockById, deleteRoom, deleteFloor } = useHallData();
  ...
  const block = floor?.blockId ? getBlockById(floor.blockId) : null;
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isSupervisor && (
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          const success = deleteRoom(floorNumber, room.id);
                          if (success) {
                            toast({ title: "Room Deleted", description: `Room ${room.id} has been deleted.` });
                          } else {
                            toast({ title: "Cannot Delete", description: "Room has students. Remove students first.", variant: "destructive" });
                          }
                        }}
                      >
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    {selectedStudent.batch && <Badge>Batch {selectedStudent.batch}</Badge>}
                    <Badge variant={selectedStudent.status === "regular" ? "default" : "secondary"} className="flex items-center gap-1">
                      <BookOpen className="w-3 h-3" />
                      {selectedStudent.status === "regular" ? "Regular" : "Irregular"}
                    </Badge>
                  </div>
                </div>
              </div>
