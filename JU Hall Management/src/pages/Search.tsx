...
export default function Search() {
  const { floors, blocks, getBlockById } = useHallData();
  const [query, setQuery] = useState("");
  const [department, setDepartment] = useState("All Departments");
  const [district, setDistrict] = useState("All Districts");
  const [showFilters, setShowFilters] = useState(false);

  // Build searchable students from hall data
  const allStudents = useMemo(() => {
    ...
    floors.forEach((floor) => {
      const block = getBlockById(floor.blockId);
  ...
  const filteredStudents = useMemo(() => {
    return allStudents.filter((student) => {
      const matchesQuery =
        ...
        student.blockName.toLowerCase().includes(query.toLowerCase());
  ...
  }, [query, department, district, allStudents]);
      ...
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-muted-foreground">
            Find students by name, room number, department, block, or district
          </p>
        </motion.div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input
                ...
                placeholder="Search by name, room, block, or department..."
